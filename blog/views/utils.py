...
class PostViewDeleteGETMixin:
    """Mixin for Delete post and View post Views"""
    delete_route = None
    view_route = None
    html_template = ""
    ...
    async def get(self):
        ...
        context = {'post': result[0], 'slug': slug, 'delete_route': self.delete_route,
                   'view_route': self.view_route, 'logged': username}
...
class ObjDeleteMixin:
    """Mixin for deletion category, post. "obj" might be db.posts or db.categories"""
    obj = None
...
class CategoryCreateUpdateMixin:
    """Mixin for category creation/update"""
    action_update = False
